...
* **Bootpay Fork**: 최신 webview_flutter_wkwebview v3.23.2 기반 마이그레이션
  - Plugin class: `WebViewFlutterPlugin` → `BTWebViewFlutterPlugin`
  - Dart plugin class: `WebKitWebViewPlatform` → `BTWebKitWebViewPlatform`
  - Swift 기반 URL 처리: NavigationDelegateProxyAPIDelegate에 통합
  - 한국 결제 앱 자동 처리: startAppToApp, startItunesToInstall 구현
  - Custom URL scheme 완벽 지원 (카카오/네이버/토스/각종 카드사)
  - App Store 자동 연결 (미설치 앱 자동 안내)
  - Objective-C에서 Swift로 전환
* Pigeon v25.3.2

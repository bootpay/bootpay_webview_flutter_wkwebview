...
NSString *const BTWKEstimatedProgressKeyPath = @"estimatedProgress";
...
- (instancetype)initWithWebView:(WKWebView *)webView channel:(FlutterMethodChannel *)channel {
  ...
  if (self) {
    ...
    [webView addObserver:self
              forKeyPath:BTWKEstimatedProgressKeyPath
...
- (void)stopObservingProgress:(WKWebView *)webView {
  [webView removeObserver:self forKeyPath:BTWKEstimatedProgressKeyPath];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:BTWKEstimatedProgressKeyPath]) {
    NSNumber *newValue =
